...
class WildMix(Dataset):
    ...
    def __init__(self, dataset_info, dataset_split, transform=None):
        self._info = dataset_info
        self._raw_data = self._load_compseq()
        self._datapoints = self._load_augmentation(self._info)
        self._transform = transform

    def __len__(self):
        assert len(self._datapoints) % self._nsrc == 0
        return len(self._datapoints) // self._nsrc

    def __getitem__(self, idx):
        ground_truths = []
        component_info = []
        nsrc = self._info['num_sources']
        ...
        for iid in instances.index:
            ...
            source_info.append(instances.at[iid, 'category'])
    ...
    def _create_gt(self, instances, iid):
        ...
        """
        mixture_duration = self._info['mixture_duration']
    ...
    @staticmethod
    def _load_augmentation(dataset_info):
